...
function blogn_mod_db_log_nextback_url($user, $key_id) {
	...
	$sqllog = mysql_query($qry, $sql_connect);
	$next = mysql_fetch_array($sqllog);
...
function blogn_mod_db_archive_count_load($user, $key_count) {
	...
	if ($user) {
		$sql = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist WHERE reserve = 0 OR (reserve = 1 AND date < '$nowdate') ORDER BY date DESC";
	}else{
		$sql = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist WHERE secret = 0 AND (reserve = 0 OR (reserve = 1 AND date < '$nowdate')) ORDER BY date DESC";
	}
...
function blogn_mod_db_log_load_for_list($uid, $start_key, $key_count) {
	...
	if ($userdata["admin"]) {
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id ORDER BY ".BLOGN_DB_PREFIX."_loglist.date DESC LIMIT $start_key, $key_count";
	}else{
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE ".BLOGN_DB_PREFIX."_loglist.user_id = $uid ORDER BY ".BLOGN_DB_PREFIX."_loglist.date DESC LIMIT $start_key, $key_count";
	}
	$result = mysql_query($qry, $sql_connect);
	$i = 0;
	$newlog[0] = false;
...
function blogn_mod_db_log_load_list_for_category($user, $start_key, $key_count, $key_category) {
	...
	if ($userdata["admin"]) {
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE ".BLOGN_DB_PREFIX."_loglist.category_id LIKE '$check_key' ORDER BY ".BLOGN_DB_PREFIX."_loglist.date DESC LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_log_load_for_viewer($user, $start_key, $key_count) {
	...
	if ($user) {
		...
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE (".BLOGN_DB_PREFIX."_loglist.reserve = 0 OR (".BLOGN_DB_PREFIX."_loglist.reserve = 1 AND ".BLOGN_DB_PREFIX."_loglist.date < '$nowdate')) AND ".BLOGN_DB_PREFIX."_loglist.secret = 0 ORDER BY ".BLOGN_DB_PREFIX."_loglist.date DESC LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_log_load_for_month($user, $start_key, $key_count, $key_date, $vorder) {
	...
	if ($user) {
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE (".BLOGN_DB_PREFIX."_loglist.reserve = 0 OR (".BLOGN_DB_PREFIX."_loglist.reserve = 1 AND ".BLOGN_DB_PREFIX."_loglist.date < '$nowdate')) AND ".BLOGN_DB_PREFIX."_loglist.date LIKE '$checkdate' ORDER BY ".BLOGN_DB_PREFIX."_loglist.date $order_mode LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_log_load_for_day($user, $start_key, $key_count, $key_date) {
	...
	if ($user) {
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE (".BLOGN_DB_PREFIX."_loglist.reserve = 0 OR (".BLOGN_DB_PREFIX."_loglist.reserve = 1 AND ".BLOGN_DB_PREFIX."_loglist.date < '$nowdate')) AND ".BLOGN_DB_PREFIX."_loglist.date LIKE '$checkdate' ORDER BY ".BLOGN_DB_PREFIX."_loglist.date DESC LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_log_load_for_category($user, $start_key, $key_count, $key_category, $vorder) {
	...
	if ($user) {
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE (".BLOGN_DB_PREFIX."_loglist.reserve = 0 OR (".BLOGN_DB_PREFIX."_loglist.reserve = 1 AND ".BLOGN_DB_PREFIX."_loglist.date < '$nowdate')) AND ".BLOGN_DB_PREFIX."_loglist.category_id LIKE '$check_key' ORDER BY ".BLOGN_DB_PREFIX."_loglist.date $order_mode LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_log_load_for_user($user, $start_key, $key_count, $key_user) {
	...
	if ($user) {
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_loglist INNER JOIN ".BLOGN_DB_PREFIX."_logdata ON ".BLOGN_DB_PREFIX."_loglist.id = ".BLOGN_DB_PREFIX."_logdata.id WHERE (".BLOGN_DB_PREFIX."_loglist.reserve = 0 OR (".BLOGN_DB_PREFIX."_loglist.reserve = 1 AND ".BLOGN_DB_PREFIX."_loglist.date < '$nowdate')) AND ".BLOGN_DB_PREFIX."_loglist.user_id = $key_user ORDER BY ".BLOGN_DB_PREFIX."_loglist.date DESC LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_log_add($user_id, $date, $reserve, $secret, $comment_ok, $trackback_ok, $category, $title, $mes, $more, $br_change) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_loglist WRITE,".BLOGN_DB_PREFIX."_logdata WRITE", $sql_connect);
	...
	$id = mysql_insert_id($sql_connect);
	...
	$result = mysql_query("UNLOCK TABLES", $sql_connect);
...
function blogn_mod_db_log_comment_delete($ent_id) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_commentlist WRITE,".BLOGN_DB_PREFIX."_commentdata WRITE", $sql_connect);

	$qry = "SELECT id FROM ".BLOGN_DB_PREFIX."_commentlist WHERE entry_id = $ent_id";
	$cmt = mysql_query($qry, $sql_connect);
	while ($row = mysql_fetch_array($cmt)) {
		$cmt_id = $row["id"];
		$qry = "DELETE FROM ".BLOGN_DB_PREFIX."_commentdata WHERE id = $cmt_id";
		$cmtdel = mysql_query($qry, $sql_connect);
...
function blogn_mod_db_log_trackback_delete($ent_id) {
	...
	$trkdel = mysql_query($qry, $sql_connect);
...
function blogn_mod_db_comment_load_for_list($ent_id, $start_key, $key_count) {
	...
	if ($key_count != 0) {
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_commentlist INNER JOIN ".BLOGN_DB_PREFIX."_commentdata ON ".BLOGN_DB_PREFIX."_commentlist.id = ".BLOGN_DB_PREFIX."_commentdata.id WHERE ".BLOGN_DB_PREFIX."_commentlist.entry_id = '$ent_id' ORDER BY ".BLOGN_DB_PREFIX."_commentlist.date DESC LIMIT $start_key, $key_count";
	}else{
...
function blogn_mod_db_comment_load_for_new($user, $start_key, $key_count) {
	...
	if ($user) {
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_commentlist INNER JOIN ".BLOGN_DB_PREFIX."_commentdata ON ".BLOGN_DB_PREFIX."_commentlist.id = ".BLOGN_DB_PREFIX."_commentdata.id  ORDER BY ".BLOGN_DB_PREFIX."_commentlist.date DESC LIMIT $start_key, $key_count";
		}else{
		...
		if ($key_count != 0) {
			$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_commentlist INNER JOIN ".BLOGN_DB_PREFIX."_commentdata ON ".BLOGN_DB_PREFIX."_commentlist.id = ".BLOGN_DB_PREFIX."_commentdata.id WHERE ".BLOGN_DB_PREFIX."_commentlist.secret = 0 ORDER BY ".BLOGN_DB_PREFIX."_commentlist.date DESC LIMIT $start_key, $key_count";
		}else{
...
function blogn_mod_db_trackback_load_for_list($ent_id, $start_key, $key_count) {
	...
	if ($key_count != 0) {
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_trackback WHERE entry_id = '$ent_id' ORDER BY date DESC LIMIT $start_key, $key_count";
	}else{
...
function blogn_mod_db_trackback_load_for_new($start_key, $key_count) {
	...
	if ($key_count != 0) {
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_trackback ORDER BY date DESC LIMIT $start_key, $key_count";
	}else{
...
function blogn_mod_db_file_load($admin, $uid, $start_key, $end_key) {
	...
	if ($admin) {
		$qry = "SELECT count(*) FROM ".BLOGN_DB_PREFIX."_filelist ORDER BY id DESC";
	}else{
		$qry = "SELECT count(*) FROM ".BLOGN_DB_PREFIX."_filelist WHERE user_id = $uid ORDER BY id DESC";
	}
	$result = mysql_query($qry, $sql_connect);
	$row = mysql_fetch_array($result);
	$list[2] = $row[0];
	...
	if ($admin) {
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_filelist ORDER BY id DESC LIMIT $start_key, $count";
	}else{
		$qry = "SELECT * FROM ".BLOGN_DB_PREFIX."_filelist WHERE user_id = $uid ORDER BY id DESC LIMIT $start_key, $count";
	}
	$result = mysql_query($qry, $sql_connect);
	$list[0] = false;
	while($row = mysql_fetch_array($result)) {
...
function blogn_mod_db_category1_add($category_name) {
	...
	$qry  = "INSERT INTO ".BLOGN_DB_PREFIX."_categorygroup(category_name) ";
	$qry .= "VALUES(";
...
function blogn_mod_db_category2_add($group_id, $category_name) {
	...
	$qry  = "INSERT INTO ".BLOGN_DB_PREFIX."_categorylist(group_id,category_name) ";
	$qry .= "VALUES(";
	$qry .= $group_id.",";
	$qry .= "'".blogn_mod_db_comma_change(blogn_mod_db_cnv_dbstr($category_name))."'";
	$qry .= ")";
...
function blogn_mod_db_category1_change($id, $updown) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_categorygroup WRITE", $sql_connect);
...
function blogn_mod_db_category2_change($c1_id, $id, $updown) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_categorylist WRITE", $sql_connect);
...
function blogn_mod_db_link_group_add($group_name) {
	...
	$qry  = "INSERT INTO ".BLOGN_DB_PREFIX."_linkgroup(group_name) ";
	$qry .= "VALUES(";
	$qry .= "'".blogn_mod_db_comma_change(blogn_mod_db_cnv_dbstr($group_name))."'";
	$qry .= ")";
	$result = mysql_query($qry, $sql_connect);
...
function blogn_mod_db_link_list_add($group_id, $link_name, $link_url) {
	...
	$qry  = "INSERT INTO ".BLOGN_DB_PREFIX."_linklist(group_id,link_name,link_url) ";
	$qry .= "VALUES(";
	$qry .= $group_id.",";
...
function blogn_mod_db_link_group_change($id, $updown) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_linkgroup WRITE", $sql_connect);
...
function blogn_mod_db_link_list_change($group_id, $id, $updown) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_linklist WRITE", $sql_connect);
...
function blogn_mod_db_user_add($loginid, $loginpw, $name, $mailaddress, $useradmin, $useractive) {
	...
	$result = mysql_query("LOCK TABLES ".BLOGN_DB_PREFIX."_userlist WRITE,".BLOGN_DB_PREFIX."_profile WRITE", $sql_connect);
...
function blogn_mod_db_user_profile_update($req_id, $user_id, $user_pw, $user_name, $user_profile, $init_comment_ok, $init_trackback_ok, $init_category, $init_icon_ok, $receive_mail_address, $receive_mail_pop3, $receive_mail_user_id, $receive_mail_user_pw, $receive_mail_apop, $access_time, $send_mail_address, $mobile_category, $mobile_comment_ok, $mobile_trackback_ok, $information_mail_address, $information_comment, $information_trackback, $user_mail_address, $br_change) {
	...
	$qry  = "UPDATE ".BLOGN_DB_PREFIX."_userlist ";
	$qry .= "SET user_id = '{$user_id}',";
	if ($user_pw) $qry .= "user_pw = '".md5($user_pw)."',";
	$qry .= "user_name = '{$user_name}',init_comment_ok = {$init_comment_ok},init_trackback_ok = {$init_trackback_ok},";
	if (!$init_category) {
...
function blogn_mod_db_user_delete($ent_id) {
	...
	$qry  = "LOCK TABLES ".BLOGN_DB_PREFIX."_userlist WRITE,";
	$qry .= BLOGN_DB_PREFIX."_profile WRITE,";
	$qry .= BLOGN_DB_PREFIX."_loglist WRITE,";
	$qry .= BLOGN_DB_PREFIX."_logdata WRITE,";
	$qry .= BLOGN_DB_PREFIX."_commentlist WRITE,";
	$qry .= BLOGN_DB_PREFIX."_commentdata WRITE,";
	$qry .= BLOGN_DB_PREFIX."_trackback WRITE";
	$result = mysql_query($qry, $sql_connect);
	...
	$log = mysql_query($qry, $sql_connect);
...
function blogn_mod_db_mobile_access() {
	...
	$qry  = "LOCK TABLES ".BLOGN_DB_PREFIX."_access WRITE,";
	$result = mysql_query($qry, $sql_connect);
...
function blogn_mod_db_viewskin_del($id) {
	...
	$qry = "DELETE FROM ".BLOGN_DB_PREFIX."_viewskin WHERE id = '{$id}' LIMIT 1";
	$result = mysql_query($qry, $sql_connect);
